...
class TriangularGrid:
        ...
        for row in range(self.rows):
            ...
            for col in range(self.cols):
                if (
                    ...
                    or col >= self.cols - distance_from_center
                ):
                    continue
                coord = (row, col)
                self.cells[coord] = TriangleCell(row, col)
    ...
    def render_svg(self, filename, cell_size=30, stroke_width=4):
        ...
        for coord, cell in self.cells.items():
            for neighbor in cell.neighbors:
                ...
                if pair not in drawn:
                    ...
                    dwg.add(
                        dwg.line(
                            (x1, y1), (x2, y2), stroke="red", stroke_width=stroke_width
                        )
                    )
                    drawn.add(pair)

    def _draw_passages(self, dwg, cell_size, stroke_width):
